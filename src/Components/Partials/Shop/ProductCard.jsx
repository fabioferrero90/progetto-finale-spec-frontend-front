  ...
  return (
    <div className="product-card flex flex-col hover:scale-105 transition-transform duration-300">
      ...
      <div className="product-info cursor-pointer ">
        <h3 className="product-name font-bold uppercase text-sm" onClick={() => navigate(`/product/${product.id}`)}>{product.title || "Titolo Prodotto"}</h3>
        <p className="product-description text-sm text-gray-600" onClick={() => navigate(`/product/${product.id}`)}>{product.description || "Descrizione Prodotto"}</p>
